...
export function PublicationFilters({ register, onFilter, onClear }: PublicationFiltersProps) {
  return (
    <div className="bg-white/60 dark:bg-neutral-900/40 backdrop-blur-md border border-slate-200 dark:border-white/5 rounded-2xl p-4 shadow-sm flex flex-wrap gap-4 items-end mb-8">
      ...
      <div className="flex gap-2 w-full md:w-auto">
        <button 
          onClick={onFilter}
          className="flex-1 md:flex-none px-6 py-2 bg-indigo-600 text-white font-bold rounded-xl shadow-md hover:bg-indigo-700 transition-colors"
        >
          Filtrar
        </button>
        <button 
          onClick={onClear}
          className="flex-1 md:flex-none px-6 py-2 bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-neutral-300 font-bold rounded-xl hover:bg-slate-200 dark:hover:bg-white/10 transition-colors"
        >
          Limpar
        </button>
      </div>
